 ...
 */
class Bdu_Passport_Passgate implements Bd_Passport_IError{
	...
	/**
	 * @brief	根据密保手机查询用户信息
	 * @author	yangliang02@baidu.com
	 * @param	var		$varSmobil		密保手机[数组]
	 * @param	array	$arrFields		待获取的字段名数组
	 * @return
	 */
	public function getUserInfoBySecureMobil($varSmobil , $arrFields) {
		$strService = 'userinfo_all';
	
		$arrUmobils = array();
		if (is_string($varSmobil)) {
			$arrUmobils = array(strval($varSmobil));
		} elseif (is_array($varSmobil)) {
			foreach($varSmobil as $strUname) {
				$arrUmobils[] = strval($strUname);
			}
		} else {
			Bd_Passport_Log::warning("[getUserInfoBySecureMobil]Invalid params.", E_USER_WARNING);
			return false;
		}
		$arrInput = $this->_prepareTemplate($strService , 'batget' , $arrFields);
		$arrInput['content'][0]['params']['securemobil'] = $arrUmobils;
		$arrInput['content'][0]['params']['get_version'] = 2;
	 ...
	 * @brief	根据密保手机查询用户信息
	 * @author	yangliang02@baidu.com
	 * @param	var		$varSemail		密保手机[数组]
	 * @param	array	$arrFields		待获取的字段名数组
	 * @return
	 */
	public function getUserInfoBySecureEmail($varSemail , $arrFields) {
		$strService = 'userinfo_all';
	
		$arrUemails = array();
		if (is_string($varSemail)) {
			$arrUemails = array(strval($varSemail));
		} elseif (is_array($varSemail)) {
			foreach($varSemail as $strUname) {
				$arrUemails[] = strval($strUname);
			}
		} else {
			Bd_Passport_Log::warning("[getUserInfoBySecureEmail]Invalid params.", E_USER_WARNING);
			return false;
		}
		$arrInput = $this->_prepareTemplate($strService , 'batget' , $arrFields);
		$arrInput['content'][0]['params']['secureemail'] = $arrUemails;
		$arrInput['content'][0]['params']['get_version'] = 2;

(function ($) {
    ...
    var methods_checker = {
        ...
        init:function (options) {
            methods_checker.settings = $.extend({}, $.fn.keyvalueeditor_checker.defaults, options);
            ...
            return this.each(function () {
                ...
                if (!data) {
                    data = {
                        settings:methods_checker.settings,
                        editor:$this
                    };

                    var h = methods_checker.getLastRow(data);
                    $this.append(h);

                    $this.on("focus.keyvalueeditor", '.keyvalueeditor-last', data, methods_checker.focusEventHandler);
                    $this.on("focus.keyvalueeditor", '.keyvalueeditor-row input', data, methods_checker.rowFocusEventHandler);
                    $this.on("blur.keyvalueeditor", '.keyvalueeditor-row input', data, methods_checker.blurEventHandler);
                    $this.on("change.keyvalueeditor", '.keyvalueeditor-valueTypeSelector ', data, methods_checker.valueTypeSelectEventHandler);
                    $this.on("click.keyvalueeditor", '.keyvalueeditor-delete', data, methods_checker.deleteRowHandler);
        ...
        getLastRow:function (state) {
            ...
            h += '<select class="keyvalueeditor-checkerTypeSelector">'
        ...
        getNewRow:function (key, value, type, state) {
            ...
            h += methods_checker.getDeleteLink(state);
            h += '</div>';
            return h;
        },
        ...
        focusEventHandler:function (event) {
            ...
            var row = methods_checker.getLastRow(event.data);
            if (event.data.settings.valueTypes.length > 1) {
                $(this).find('select:last').after(methods_checker.getDeleteLink(event.data));
            }
            else {
                $(this).find('input:last').after(methods_checker.getDeleteLink(event.data));
            }
        ...
        addParam:function (param, state) {                
            ...
            $(state.editor).find('.keyvalueeditor-last').before(methods_checker.getNewRow(param.key, param.value, param.type, state));
        },
        ...
        addParams:function (params, state) {
            ...
            for (var i = 0; i < count; i++) {
                var param = params[i];
                methods_checker.addParam(param, state);
            }
        },
        ...
        getValues:function () {
            ...
            $(this).find('.keyvalueeditor-row').each(function () {
                ...
                var checkerType = $(this).find('.keyvalueeditor-checkerTypeSelector').val();
        ...
        clear:function (state) {
            ...
            var h = methods_checker.getLastRow(state);
            $(state.editor).append(h);
        },

        reset:function (params) {
            var state = $(this).data('keyvalueeditor');
            methods_checker.clear(state);
            if (params) {
                methods_checker.addParams(params, state);
            }
    ...
    $.fn.keyvalueeditor_checker = function (method) {
        //Method calling logic
        if (methods_checker[method]) {
            return methods_checker[method].apply(this, Array.prototype.slice.call(arguments, 1));
        } else if (typeof method === 'object' || !method) {
            return methods_checker.init.apply(this, arguments);
        } else {
            $.error('Method ' + method + ' does not exist on jQuery.keyvalueeditor');
        }
    };

    $.fn.keyvalueeditor_checker.defaults = {
        type:"normal",
        fields:2,
        deleteButton:"Delete",
        placeHolderKey:"Key",
